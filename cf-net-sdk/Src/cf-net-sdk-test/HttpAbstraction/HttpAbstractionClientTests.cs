...
namespace CloudFoundry.Test.HttpAbstraction
{
    [TestClass]
    public class HttpAbstractionClientTests
    {
        ...
        [TestCategory("Integration")]
        [TestCategory("LongRunning")]
        public void CanMakePostWithMultiPartDataRequest()
        {
            using (var client = new HttpAbstractionClientFactory().Create())
            {
                using (var content = TestHelper.CreateStream("Test Text"))
                {
                    using (var otherContent = TestHelper.CreateStream("Other Test Text"))
                    {
                        client.Uri = new Uri("http://httpbin.org/post");
                        client.Method = HttpMethod.Post;

                        var parts = new List<IHttpMultiPartFormDataAbstraction>();
                        parts.Add(new HttpMultiPartFormDataAbstraction("field1", "file1.zip","application/zip", content));
                        parts.Add(new HttpMultiPartFormDataAbstraction("field2", "file2.zip", "application/zip", otherContent));

                        var responseTask = client.SendAsync(parts);
                        responseTask.Wait();
                        var response = responseTask.Result;
                        ...
                        var stringContent = TestHelper.GetStringFromStream(response.Content);
                        Assert.IsTrue(stringContent.Contains("\"field1\": \"Test Text\""));
                        Assert.IsTrue(stringContent.Contains("\"field2\": \"Other Test Text\""));
                        Assert.IsTrue(stringContent.Contains("\"Content-Type\": \"multipart/form-data;"));
                    }
                }
            }
        }
